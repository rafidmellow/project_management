// scripts/update-edge-permissions-admin-only.js
// Script to update edge-permission-service.ts with admin permissions
...
async function updateEdgePermissions() {
  console.log('Starting edge permissions update for admin only...');

  try {
    // Get admin role with permissions
    console.log('Fetching admin role and permissions from database...');
    ...
    const rolePermissions = {
      admin: adminRole.permissions.map((rp) => rp.permission.name),
    };
